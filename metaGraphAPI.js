...
class MetaGraphAPI {
  ...
  // ================================================
  // MESSAGING METHODS (Phase 2 - AI Lead Automation)
  // ================================================
  // Added: 2026-06-17
  // Purpose: Send AI-generated responses to Facebook Messenger and Instagram DMs
  // Status: INACTIVE until Phase 3 webhook endpoints are deployed
  // ================================================

  /**
   * Send message via Facebook Messenger
   * @param {string} recipientPSID - Page-Scoped ID of recipient
   * @param {string} messageText - Message text to send
   * @returns {object} - Meta API response
   */
  async sendFacebookMessage(recipientPSID, messageText) {
    if (!this.accessToken || !this.facebookPageId) {
      throw new Error('Facebook credentials not configured');
    }
    ...
    try {
      ...
      console.log('✅ Facebook Messenger reply sent:', response.data.message_id);
      ...
      console.error('❌ Facebook Messenger send failed:', error.response?.data || error.message);
      throw new Error(`Failed to send Facebook message: ${error.response?.data?.error?.message || error.message}`);
    }
  }

  /**
   * Send message via Instagram Messaging
   * @param {string} recipientIGID - Instagram-Scoped ID of recipient
   * @param {string} messageText - Message text to send
   * @returns {object} - Meta API response
   */
  async sendInstagramMessage(recipientIGID, messageText) {
    if (!this.accessToken || !this.instagramAccountId) {
      throw new Error('Instagram credentials not configured');
    }
    ...
    try {
      ...
      console.log('✅ Instagram DM reply sent:', response.data.message_id);
  ...
  async markMessageAsRead(senderId, platform = 'facebook') {
    ...
    try {
      ...
      console.log(`✅ Message marked as read (${platform}):`, senderId);
      ...
      console.error(`❌ Mark as read failed (${platform}):`, error.response?.data || error.message);
  ...
  /**
   * Send typing indicator (shows "..." while AI is generating response)
  ...
  async sendTypingIndicator(recipientId) {
    ...
    try {
      ...
      console.log('✅ Typing indicator sent:', recipientId);
