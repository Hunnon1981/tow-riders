...
const FROM_EMAIL = process.env.FROM_EMAIL || 'services@towriders.com';
...
async function sendBookingEmail(bookingData) {
  ...
  try {
    ...
    const businessEmailResult = await sgMail.send(businessEmailMsg);
    
    console.log('✅ Business email sent successfully via SendGrid!');
    console.log('Status Code:', businessEmailResult[0]?.statusCode);
    ...
    return { success: false, message: error.message, errorDetails: error.response?.body };
  }
}
...
async function sendPaymentConfirmationEmail(session) {
  ...
  try {
    console.log('📧 Attempting to send payment confirmation email via SendGrid...');
    console.log('From:', FROM_EMAIL);
    console.log('To:', BUSINESS_EMAIL);
    ...
    console.log('✅ Payment confirmation email sent successfully via SendGrid!');
    console.log('Status Code:', result[0]?.statusCode);
  } catch (error) {
...
async function sendQuoteEmail(quoteData) {
  ...
  try {
    console.log('📧 Attempting to send quote request email via SendGrid...');
    console.log('From:', FROM_EMAIL);
    console.log('To:', BUSINESS_EMAIL);
...
app.post('/api/partner', async (req, res) => {
  try {
    ...
    if (process.env.SENDGRID_API_KEY) {
      console.log('📧 Attempting to send partner application email via SendGrid...');
      console.log('From:', FROM_EMAIL);
      console.log('To:', BUSINESS_EMAIL);
      ...
      console.log('✅ Partner email sent successfully via SendGrid!');
      console.log('Status Code:', result[0]?.statusCode);
    }
...
app.get('/api/test-email', async (req, res) => {
  try {
    console.log('🧪 EMAIL TEST STARTED (SendGrid)');
    ...
    console.log('  ✓ First 8 Characters:', apiKey ? apiKey.substring(0, 8) : 'N/A');
    console.log('  ✓ Total Key Length:', apiKey ? apiKey.length : 0);
    console.log('  ✓ Starts with "SG.":', apiKey ? apiKey.startsWith('SG.') : false);
    console.log('  ✓ Environment Variable Name Used: SENDGRID_API_KEY');
    ...
    if (!apiKey) {
      return res.json({
        ...
        debug: {
          ...
          suggestion: 'Check Railway environment variables for SENDGRID_API_KEY'
        },
        config: {
    ...
    console.log('📤 Attempting test email send via SendGrid...');
    
    const msg = {
      from: FROM_EMAIL,
      to: BUSINESS_EMAIL,
      subject: 'Test Email from Tow Riders Backend (SendGrid)',
      html: `
        <h2>🧪 Test Email</h2>
        ...
        <p><strong>To:</strong> ${BUSINESS_EMAIL}</p>
    ...
    // SendGrid returns success with status code 202
    if (result && result[0]?.statusCode === 202) {
      ...
      // Unexpected response
      console.error('❌ Unexpected SendGrid response:', result);
      return res.status(500).json({
        success: false,
        message: 'Unexpected SendGrid response',
        debug: {
          apiKeyExists: true,
          apiKeyFirstChars: apiKey.substring(0, 8),
          apiKeyLength: apiKey.length,
    ...
    res.status(500).json({
      ...
      debug: {
        apiKeyExists: !!apiKey,
        apiKeyFirstChars: apiKey ? apiKey.substring(0, 8) : 'N/A',
        apiKeyLength: apiKey ? apiKey.length : 0,
        startsWithSG: apiKey ? apiKey.startsWith('SG.') : false,
        environmentVariableName: 'SENDGRID_API_KEY'
      },
      config: {
...
// ================================================
// META WEBHOOK ROUTES (Phase 3 - AI Lead Automation)
// ================================================
// Handles incoming Facebook Messenger and Instagram DM webhooks
// Test mode until OPENAI_API_KEY is configured
// Webhook URL: https://tow-riders-production.up.railway.app/api/meta/webhook

try {
  const metaWebhookRoutes = require('./metaWebhook');
  app.use('/api/meta/webhook', metaWebhookRoutes);
  console.log('✅ Meta webhook routes loaded (Test mode - configure OPENAI_API_KEY to enable AI)');
} catch (error) {
  console.error('❌ META WEBHOOK LOAD ERROR:', error.message);
...
app.listen(PORT, () => {
  ...
  console.log('   GET  /api/meta/webhook/status');
  console.log('   GET  /api/meta/webhook');
  console.log('   POST /api/meta/webhook');
